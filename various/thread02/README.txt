...
Topic: Interrupt monitored for to end thread, and not monitored and thread wont end
       Non-blocking threads

Programs in this project:

Main1.java - simple example with two threads - one that responds to interruption and the other that does not
			 - interruption handled using while (!Thread.currentThread().isInterrupted()) 
Main2.java - interruption handled using InterruptedException
